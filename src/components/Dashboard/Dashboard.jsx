...
const Dashboard = () => {
    ...
    const handleDownload = async (id) => {
        try {
            // Open the CV in a new window for PDF generation
        ...
        <html>
          <head>
            ...
            <style>
              body { margin: 0; padding: 20px; background: #ecf0f1; }
              .cv-preview-wrapper { background: white; max-width: 210mm; margin: 0 auto; }
            </style>
          </head>
          <body>
            <div id="cv-container"></div>
            <script>
              window.cvData = ${JSON.stringify(cv)};
            </script>
          </body>
        </html>
      `);

            // Navigate to the CV view for download
            window.open(`/cv/${id}`, '_blank');
            alert('CV opened in new tab. Use your browser\'s print function (Ctrl+P) to save as PDF.');
        } catch (error) {
